...
class MobiDukaDemo extends StatelessWidget {
    ...
    return MaterialApp(
      ...
      home: MobiDukaApp(
        ...
        screenBuilders: {
          ...
          AppScreen.pos: (context, data, onNavigate) =>
              _buildProducts('Point of Sale', data.products),
          AppScreen.inventory: (context, data, onNavigate) =>
              _buildProducts('Inventory & Stock', data.products),
          AppScreen.customers: _buildCustomers,
          AppScreen.reports: (context, data, onNavigate) => _buildDetailPage(
            'Reports & Analytics',
            Icons.bar_chart,
            data,
            onNavigate,
          ),
          AppScreen.settings: (context, data, onNavigate) => _buildDetailPage(
            'Settings',
            Icons.settings_outlined,
            data,
            onNavigate,
          ),
          AppScreen.profile: (context, data, onNavigate) => _buildDetailPage(
            'User Profile',
            Icons.person_outline,
            data,
            onNavigate,
          ),
          AppScreen.purchases: (context, data, onNavigate) => _buildDetailPage(
            'Purchase Orders',
            Icons.receipt_long_outlined,
            data,
            onNavigate,
          ),
          AppScreen.suppliers: (context, data, onNavigate) => _buildDetailPage(
            'Suppliers',
            Icons.factory_outlined,
            data,
            onNavigate,
          ),
          AppScreen.credit: (context, data, onNavigate) => _buildDetailPage(
            'Credit Book',
            Icons.menu_book_outlined,
            data,
            onNavigate,
          ),
          AppScreen.expenses: (context, data, onNavigate) => _buildDetailPage(
            'Expense Tracking',
            Icons.payments_outlined,
            data,
            onNavigate,
          ),
          AppScreen.employees: (context, data, onNavigate) => _buildDetailPage(
            'Employees',
            Icons.badge_outlined,
            data,
            onNavigate,
          ),
          AppScreen.notifications: (context, data, onNavigate) =>
              _buildDetailPage(
                'Notifications',
          ...
          AppScreen.backup: (context, data, onNavigate) => _buildDetailPage(
            'Backup & Cloud Sync',
  ...
  static Widget _buildDashboard(
    ...
    return Column(
      children: [
        // Keep the store context and quick-stat strip visible while the
        // operational dashboard content below it scrolls.
        Container(
          width: double.infinity,
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                '${data.location} · Shift: ${data.shift}',
              ...
              SizedBox(
                ...
                child: ListView.separated(
                  ...
                  itemCount: data.quickStats.length,
                  separatorBuilder: (_, _) => const SizedBox(width: 8),
                  itemBuilder: (context, index) {
                    final stat = data.quickStats[index];
                    return Container(
                      width: 118,
        ...
        ...data.customers.map(
          (customer) => Card(
            child: ListTile(
              ...
              trailing: Text(
                customer.credit == 0 ? 'Settled' : 'KSh ${customer.credit}',
              ),
            ),
          ),
        ),
      ...
      children: [
        ...
        ...links.entries.map(
          (entry) => ListTile(
            ...
            trailing: entry.key == 'Notifications'
  ...
  static Widget _buildDetailPage(
    String title,
    IconData icon,
    AppData data,
    void Function(String screen) onNavigate,
...
class _ManagementPageState extends State<ManagementPage> {
      ...
      children: [
        ...
        Expanded(
          child: LinearProgressIndicator(
            value: value / 100,
            minHeight: 8,
            color: current ? const Color(0xFFD4AF37) : const Color(0xFF123A8F),
  ...
  List<Widget> _purchaseOrders() {
    ...
    return [
      ...
      ...orders.where((order) => filter == 'All' || order.$3 == filter).map((
        order,
      ) {
        return Card(
          child: ListTile(
  ...
  List<Widget> _suppliers() {
    ...
    return [
      const _SummaryStrip(
        items: [('Total Suppliers', '6'), ('Outstanding', 'KSh 93K')],
      ),
      const TextField(
  ...
  List<Widget> _creditBook() => [
    const _SummaryStrip(
      items: [('Outstanding Credit', 'KSh 22,100'), ('Accounts', '5 active')],
    ),
    ...widget.data.customers
        ...
        .map(
          (customer) => Card(
            child: ListTile(
              ...
              title: Text(customer.name),
              subtitle: Text(
                '${customer.phone} · ${customer.purchases} transactions\nLast: ${customer.lastVisit}',
              ),
              isThreeLine: true,
              trailing: Text(
                'KSh ${customer.credit}',
  ...
  List<Widget> _employees() {
    ...
    return [
      const _SummaryStrip(
        items: [('Total', '5'), ('Active', '4'), ('Inactive', '1')],
      ),
      ...employees.map(
        (employee) => Card(
          child: ListTile(
            ...
            trailing: Column(
              ...
              children: [
                Text(employee.$4),
                Text(
                  employee.$5 ? '● Active' : '● Inactive',
                  style: TextStyle(
                    color: employee.$5 ? Colors.green : Colors.red,
                    fontSize: 11,
                  ),
                ),
              ],
  ...
  List<Widget> _notifications() => [
    ...
    ...[
      (
        ...
        'Panadol 500mg has only 3 units remaining.',
        '14:30',
      ),
      (
        '⚠️',
        'Low Stock Warning',
        'Royco 75g is running low (5 units).',
        '13:55',
      ),
      (
        '🎯',
        'Daily Sales Target Achieved',
        'Today\'s sales of KSh 84,250 exceeded target.',
        '13:00',
      ),
      (
        '📦',
        'New Purchase Order Received',
        'PO-2026-083 from Bidco Africa delivered.',
        '11:20',
      ),
      (
        '☁️',
      ...
      (notice) => Card(
        child: ListTile(
          leading: Text(notice.$1, style: const TextStyle(fontSize: 22)),
          title: Text(notice.$2),
          subtitle: Text('${notice.$3}\n${notice.$4}'),
          isThreeLine: true,
        ),
      ),
    ),
  ...
  List<Widget> _backup() => [
    const Card(
      child: ListTile(
        ...
        subtitle: Text('Last backup: Today, 06:00 AM\nAll data synced'),
      ),
    ),
    ...
    SwitchListTile(
      ...
      subtitle: const Text('Backup automatically every day at 6:00 AM'),
      value: autoBackup,
      ...
      const Column(
        children: [
          ListTile(
            ...
            title: Text('Today, 06:00 AM'),
            subtitle: Text('12,450 records · 4.2 MB'),
          ),
          ListTile(
            leading: Icon(Icons.check_circle, color: Colors.green),
            title: Text('Yesterday, 06:00 AM'),
            subtitle: Text('12,287 records · 4.1 MB'),
          ),
          ListTile(
            leading: Icon(Icons.error, color: Colors.red),
            title: Text('5 Jul, 06:00 AM'),
            subtitle: Text('Backup failed — retried'),
          ),
        ],
      ),
    ),
      ...
      const Column(
        children: [
          ListTile(
            ...
            subtitle: Text('MobiDuka Store'),
          ),
          ListTile(
            title: Text('Location'),
            subtitle: Text('Nairobi CBD, Kenya'),
          ),
          ListTile(title: Text('Phone'), subtitle: Text('+254 712 345 678')),
          ListTile(
            title: Text('Currency'),
            subtitle: Text('KES (Kenyan Shilling)'),
          ),
        ],
      ),
    ),
    _panel(
      ...
      Column(
        children: [
          ...
          SwitchListTile(
            title: const Text('Daily Report Email'),
            value: false,
            onChanged: (_) {},
          ),
          SwitchListTile(
    ...
    _panel(
      ...
      const Column(
        children: [
          ...
          ListTile(
            leading: Text('📋'),
            title: Text('Credit / Tab'),
            trailing: Text('Active'),
          ),
          ListTile(
            leading: Text('🏦'),
            title: Text('Bank Transfer'),
            trailing: Text('Inactive'),
          ),
        ],
      ),
    ),
  ...
  List<Widget> _profile() => [
    Row(
      children: [
        const CircleAvatar(radius: 30, child: Text('A')),
    ...
    _panel(
      ...
      const Column(
        children: [
          ListTile(title: Text('Store Name'), subtitle: Text('MobiDuka Store')),
          ListTile(title: Text('Branch'), subtitle: Text('Nairobi CBD')),
        ],
      ),
    ),
    if (editing)
